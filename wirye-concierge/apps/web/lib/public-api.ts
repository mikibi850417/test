...
const CACHE_TTL_MS = Number.isFinite(CACHE_TTL_SECONDS) && CACHE_TTL_SECONDS > 0 ? CACHE_TTL_SECONDS * 1000 : 300000;
...
async function safeFetch<T>(url: string): Promise<T | null> {
  ...
  const now = Date.now();
  const fallback =
    cached && now - cached.savedAt <= CACHE_TTL_MS ? (cached.payload as T) : null;
...
export async function fetchPublicPath<T>(path: string): Promise<T | null> {
  ...
  return safeFetch<T>(`${base}/api/v1/public/hotels/${DEFAULT_HOTEL_ID}${path}`);
}
